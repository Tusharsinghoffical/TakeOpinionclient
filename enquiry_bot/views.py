...
from .models import Enquiry, FAQ, ChatMessage, ChatbotSearchSession
...
def analyze_medical_report(request):
    ...
    if request.method == 'POST' and request.FILES.get('medical_report'):
        ...
        if any(word in analysis_lower for word in ['heart', 'cardiac', 'cardiology', 'cardiovascular', 'cardiac surgery', 'hypertension', 'blood pressure']):
            ...
            # Find oncologists
            doctor_queryset = Doctor.objects.prefetch_related('hospitals').filter(
                Q(specialization__icontains='oncology') | Q(specialization__icontains='cancer')
            )[:3]
            matched_condition = True
        elif any(word in analysis_lower for word in ['stomach', 'intestine', 'bowel', 'gastro', 'colon']):
            # Find gastroenterologists
            doctor_queryset = Doctor.objects.prefetch_related('hospitals').filter(
                Q(specialization__icontains='gastroenterology') | Q(specialization__icontains='gastro')
            )[:3]
            matched_condition = True
        elif any(word in analysis_lower for word in ['kidney', 'urinary', 'bladder', 'urology']):
            # Find urologists/nephrologists
            doctor_queryset = Doctor.objects.prefetch_related('hospitals').filter(
                Q(specialization__icontains='urology') | Q(specialization__icontains='nephrology')
            )[:3]
            matched_condition = True
        elif any(word in analysis_lower for word in ['skin', 'rash', 'dermatitis', 'eczema', 'psoriasis']):
            # Find dermatologists
            doctor_queryset = Doctor.objects.prefetch_related('hospitals').filter(
            ...
            # General fallback — return a broad set of doctors
            doctor_queryset = Doctor.objects.prefetch_related('hospitals').all()[:3]
            matched_condition = True

        if matched_condition:
            for doctor in doctor_queryset:
                hospitals = [h.name for h in doctor.hospitals.all()]
...
def extract_medical_analysis(report_text: str, intent) -> dict:
    """
    Extract structured medical information from report text.
    Returns a dict with diagnosis, conditions, precautions, medicines, recommendations.
    """
    ...
    for line in report_text.splitlines():
        ...
        if any(kw in line_lower for kw in ['diagnosis:', 'diagnosed with', 'impression:', 'assessment:', 'findings:']):
            # Take the content after the colon
            for kw in ['diagnosis:', 'impression:', 'assessment:', 'findings:']:
                if kw in line_lower:
                    parts = line_stripped.split(':', 1)
                    if len(parts) > 1 and parts[1].strip():
                        diagnosis_lines.append(parts[1].strip())
                    break
    if diagnosis_lines:
        analysis['diagnosis'] = diagnosis_lines[:3]

    # --- Detected conditions from intent ---
    if intent.conditions:
        analysis['detected_conditions'] = intent.conditions

    # --- Medications mentioned in report ---
    medicine_keywords = [
        'metformin', 'glimepiride', 'insulin', 'aspirin', 'amlodipine',
        'atorvastatin', 'lisinopril', 'levothyroxine', 'omeprazole',
        'pantoprazole', 'ramipril', 'losartan', 'atenolol', 'warfarin',
        'clopidogrel', 'furosemide', 'spironolactone', 'prednisolone',
        'dexamethasone', 'amoxicillin', 'azithromycin', 'ciprofloxacin',
    ...
    for med in medicine_keywords:
        if med in text_lower:
            found_medicines.append(med.title())
    # Also extract lines with "mg" or "tablet" or "capsule"
    for line in report_text.splitlines():
        line_stripped = line.strip()
        if not line_stripped:
            continue
        line_lower = line_stripped.lower()
        if any(kw in line_lower for kw in ['mg ', 'mg\n', 'tablet', 'capsule', 'injection', 'syrup']):
            if len(line_stripped) < 100:  # avoid long lines
                found_medicines.append(line_stripped)
    # Deduplicate — prefer the detailed line over the keyword-only match
    seen = set()
    unique_medicines = []
    for m in found_medicines:
        key = m.lower().split()[0][:10]  # deduplicate by first word
        if key not in seen:
            seen.add(key)
            unique_medicines.append(m)
    if unique_medicines:
        analysis['medications'] = unique_medicines[:8]

    # --- Precautions based on conditions ---
    precaution_map = {
        'diabetes': [
            'Monitor blood sugar levels daily',
            'Follow a low glycemic index diet — avoid sugar, white rice, maida',
            'Exercise at least 30 minutes daily (brisk walking, cycling)',
        ...
        'cardiac': [
            ...
            'Take prescribed medications regularly — do not stop without consulting doctor',
            'Avoid smoking and alcohol completely',
            'Manage stress through yoga or meditation',
            'Attend all cardiac follow-up appointments',
        ],
        'hypertension': [
        ...
        'knee': [
            ...
            'Do prescribed physiotherapy exercises daily',
            'Use walking aids as recommended',
            'Apply ice packs for 15-20 minutes to reduce swelling',
            'Maintain healthy weight to reduce joint stress',
            'Avoid sitting cross-legged or squatting',
        ],
        'bone': [
            'Ensure adequate calcium (1000-1200mg/day) and Vitamin D intake',
            'Avoid falls — use non-slip footwear and handrails',
            'Do weight-bearing exercises as advised',
            'Avoid smoking and excessive alcohol',
        ],
        'brain': [
            'Avoid strenuous physical activity',
            'Take anti-seizure medications as prescribed',
            'Avoid driving until cleared by neurologist',
            'Get adequate rest — 8 hours of sleep',
            'Attend all neurology follow-up appointments',
            'Avoid stress and maintain calm environment',
        ],
        'tumor': [
            'Follow oncologist treatment plan strictly',
            'Maintain good nutrition — high protein diet',
            'Stay hydrated during chemotherapy/radiation',
            'Report any new symptoms immediately to doctor',
            'Avoid infections — maintain hygiene, avoid crowds',
        ],
        'cancer': [
            'Follow oncologist treatment plan strictly',
            'Maintain good nutrition — high protein, high calorie diet',
        ...
        'liver': [
            ...
            'Follow low-sodium diet to manage fluid retention',
            'Take prescribed medications — do not take OTC drugs without consulting doctor',
            'Monitor weight daily — sudden increase may indicate fluid buildup',
            'Avoid raw or undercooked food',
            'Get vaccinated for Hepatitis A and B',
        ],
        'thyroid': [
            'Take levothyroxine on empty stomach, 30 minutes before breakfast',
            'Do not take calcium or iron supplements within 4 hours of thyroid medication',
            'Get TSH tested every 6-8 weeks until stable',
            'Avoid excessive iodine intake',
            'Maintain consistent medication timing',
        ],
        'kidney': [
            'Restrict fluid intake as advised by nephrologist',
            'Follow low-potassium, low-phosphorus diet',
            'Avoid NSAIDs (ibuprofen, diclofenac) — they harm kidneys',
            'Monitor blood pressure and blood sugar strictly',
            'Attend all dialysis sessions if prescribed',
        ],
        'lung': [
            'Avoid smoking and secondhand smoke completely',
            'Avoid air pollution — wear N95 mask outdoors',
            'Take prescribed inhalers and medications regularly',
            'Do breathing exercises as advised by physiotherapist',
    ...
    for condition in intent.conditions:
        if condition in precaution_map:
            precautions.extend(precaution_map[condition])
    # Deduplicate
    precautions = list(dict.fromkeys(precautions))
    if precautions:
        analysis['precautions'] = precautions[:8]

    # --- General recommendations ---
    recommendations = []
    if intent.specializations:
        specs = list(dict.fromkeys(intent.specializations))[:3]
        recommendations.append(f"Consult a {specs[0]} at the earliest")
    if intent.conditions:
        recommendations.append("Carry all previous medical reports and test results to your appointment")
        recommendations.append("Do not self-medicate — follow only prescribed treatment")
    if 'diabetes' in intent.conditions:
        recommendations.append("Get annual eye check-up (diabetic retinopathy screening)")
        recommendations.append("Get annual kidney function test (creatinine, microalbumin)")
    if any(c in intent.conditions for c in ['cancer', 'tumor', 'brain']):
        recommendations.append("Seek second opinion from a major cancer centre if needed")
        recommendations.append("Discuss palliative care options with your oncologist")
    if recommendations:
...
def smart_results_view(request):
    """
    Smart results page view.

    GET  /enquiries/results/?q=<query>  — render results for a shareable link.
    POST /enquiries/results/            — process a new query and/or file upload.

    Requirements: 1.3, 1.4, 3.1, 3.2, 3.3, 3.4, 4.8, 4.9, 7.5,
                  8.1, 8.2, 8.5, 9.1, 9.2, 9.4, 11.1–11.5
    """
    warnings = []
    report_summary = ""
    ...
    if request.method == 'POST':
        ...
        uploaded_file = request.FILES.get('medical_report', None)
    else:
        query_text = request.GET.get('q', '')
        uploaded_file = None

    # ------------------------------------------------------------------
    # Step 2: Parse report if a file was uploaded
    # ------------------------------------------------------------------
    if uploaded_file is not None:
        # Requirement 4.8 / 11.1 — file size check
        if uploaded_file.size > MAX_REPORT_SIZE_BYTES:
            warnings.append(
                "Your file exceeds the 10 MB limit and could not be processed. "
                "Please upload a smaller file."
            )
            uploaded_file = None  # skip parsing

        # Requirement 11.2 — content-type validation
        elif uploaded_file.content_type not in ALLOWED_REPORT_CONTENT_TYPES:
            warnings.append(
                "We couldn't read your file format. Please upload PDF, JPG, or PNG."
            )
            uploaded_file = None  # skip parsing

    if uploaded_file is not None:
        # Requirement 4.9 — parse inside try/except ReportParseError
        try:
            report_text = ReportParser().parse(uploaded_file)
            report_summary = report_text[:500]
            # Append parsed text to the query so intent extraction benefits from it
            query_text = (query_text + ' ' + report_text).strip()
        ...
        try:
            ChatbotSearchSession.objects.update_or_create(
                ...
                defaults={
                    ...
                    'extracted_keywords': intent.keywords,
                    'extracted_conditions': intent.conditions,
                    'report_text_summary': report_summary[:500],
                },
            )
        except DatabaseError as exc:
            logger.error("Failed to save ChatbotSearchSession: %s", exc)

    # ------------------------------------------------------------------
    # Step 6: Aggregate site content (Requirement 8.1)
    # ------------------------------------------------------------------
    try:
        results = SiteContentAggregator().aggregate(intent)
    except DatabaseError as exc:
        logger.error("DatabaseError during SiteContentAggregator.aggregate: %s", exc)
        # Fallback: build an all-placeholder context so the page still renders
        empty_intent = MedicalIntent(
            keywords=[],
            conditions=[],
            specializations=[],
            raw_query=query_text,
        )
        placeholders = PlaceholderRegistry().get_placeholders({})
        results = SmartResultsContext(
    ...
    # Attach report_summary to results context
    results.report_summary = report_summary
    ...
    context = {
        ...
        'report_summary': report_summary,
        'warnings': warnings,
        'medical_analysis': extract_medical_analysis(report_summary, intent) if report_summary else {},
    }
    return render(request, 'enquiry_bot/smart_results.html', context)


@require_POST
def smart_results_api(request):
    """
    JSON API endpoint for smart results.

    POST /enquiries/results/api/
    Body (form-encoded): query=<text>

    Returns a JSON object with serialized doctors, hospitals, treatments,
    blogs, placeholders, and query_summary.

    Requirements: 9.3
    """
    query_text = request.POST.get('query', '')

    # Merge with session context (same as smart_results_view)
    ...
    try:
        ...
        logger.error("DatabaseError during smart_results_api aggregate: %s", exc)
        placeholders = PlaceholderRegistry().get_placeholders({})
        empty_intent = MedicalIntent(
            keywords=[],
            conditions=[],
