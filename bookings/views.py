...
def appointment_booking(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Get or create a fallback treatment for simple appointments
            try:
                category = TreatmentCategory.objects.get(name="General")
            except TreatmentCategory.DoesNotExist:
                category = TreatmentCategory.objects.create(name="General", type="medical")

            treatment, _ = Treatment.objects.get_or_create(
                name="General Appointment",
                defaults={"description": "General appointment booking", "category": category, "starting_price": 0.00}
            )

            # Create booking with minimal information
            booking = Booking(
                treatment=treatment,  # Always a valid Treatment object
