...
if /i "%confirm%"=="yes" (
    ...
    for /d %%i in (..\accounts\migrations\*, ..\blogs\migrations\*, ..\bookings\migrations\*, ..\core\migrations\*, ..\doctors\migrations\*, ..\feedbacks\migrations\*, ..\hospitals\migrations\*, ..\treatments\migrations\*) do (
        for %%j in (%%i\*.py) do (
            if not "%%~nj"=="__init__" (
                del "%%j"
            )
