...
def home(request: HttpRequest) -> HttpResponse:
    # Get only specific treatment categories for the tags section
    # Limit to the most important categories to avoid overwhelming users
    treatment_categories = TreatmentCategory._default_manager.filter(  # type: ignore
        name__in=[
            "Orthopedics", 
            "Cosmetic Surgery", 
            "Ayurveda", 
    ...
    context: dict[str, Any] = {
        "top_treatments": Treatment.objects.select_related('category').all()[:6],  # type: ignore
