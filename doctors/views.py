...
def doctor_media_manage(request):
    # Get the doctor object using reliable email-first lookup, then exact name match
    doctor = None
    user_email = request.user.email
    if user_email:
        doctor = Doctor.objects.filter(email__iexact=user_email).first()  # type: ignore
    if not doctor:
        user_full_name = f"{request.user.first_name} {request.user.last_name}".strip()
        if user_full_name:
            doctor = Doctor.objects.filter(name__iexact=user_full_name).first()  # type: ignore
    # No fallback to Doctor.objects.first() — if still None, show a message and redirect
    if not doctor:
